  ...
  $("#update").click(async function (e) {
    ...
    $.post(
      ...
      if (data.length > 0) {
        ...
        $.post(
          `http://localhost:3000/updateScreening/${ID}`,
          updateScreening
        ).done(function () {
          location.replace("http://localhost:3000/screening/screening.html");
        });
...
function getFilmData(FilmID) {
  ...
  $.getJSON(`http://localhost:3000/films`, function (data) {
    $.each(data, function (i, value) {
      ...
      $(`#filmSelect`).append(
        `<option value="${value.FilmID}" ${isSelected}>${value.Name}</option>`
      );
    });
  });
}
  ...
  $.getJSON(`http://localhost:3000/Theatres`, function (data) {
    $.each(data, function (i, value) {
      ...
      $(`#theatreSelect`).append(
        `<option value="${value.TheatreID}" ${isSelected}>${value.TheatreID}</option>`
      );
    });
  });
}
...
function getStartTime(StartTime) {
  $.getJSON(`http://localhost:3000/startTimes`, function (data) {
    $.each(data, function (i, value) {
      ...
      $(`#startTime`).append(
        `<option value="${value.StartTime}" ${isSelected}>${value.StartTime}</option>`
      );
    });
  });
}
